...
[![Fabric](https://img.shields.io/badge/Fabric%20Loader-%3E%3D0.16.10-9d4edd.svg)](https://fabricmc.net/)
[![Java](https://img.shields.io/badge/Java-%3E%3D21-F89820.svg)](https://www.oracle.com/java/)
...
> 本目录为 **Minecraft 1.21.1** 移植版（由 26.1 版本移植而来，使用 Yarn mappings）。
...
- **Minecraft**：1.21.1（正式版）
- **Java**：JDK 21
- **Fabric Loader**：0.16.10+（推荐）

### 依赖 Mod

| Mod | 版本 | 说明 |
|-----|------|------|
| Fabric API | 0.110.0+1.21.1 | 必需 |
| Cloth Config | 15.0.140 | 配置界面必需 |
| ModMenu | 11.0.3 | 可选（推荐，可在模组菜单中配置） |

### 安装步骤

1. 安装 [Fabric Loader](https://fabricmc.net/use/) 0.16.10+
2. 将 `portal-preview-2.0.0.jar` 放入 `.minecraft/mods/` 文件夹
3. 安装 Fabric API 0.110.0+1.21.1
4. 安装 Cloth Config 15.0.140（配置界面必需）
5. （可选）安装 ModMenu 11.0.3
6. 启动游戏，享受地狱门预览功能！
...
- 1.21.1 的 `RenderLayer.getLines()` 线宽固定为 1px（26.1 的 per-vertex 线宽 API 不存在），轮廓线可能比 26.1 版本细一些
- 如遇到渲染异常（线条不显示、颜色异常等），请向作者反馈
