...
public class PortalHudOverlay {
    ...
    /**
     * 1.20.1: 通过 HudRenderCallback.EVENT 调用（替代 26.1+ 的 HudElement）
     */
    public static void render(DrawContext drawContext) {
        ...
        if (client.world.getRegistryKey() == World.NETHER) {
            ...
            if (PortalPreviewClient.currentNetherFrame == null) {
                ...
                // 1.20.1: BlockPos.getCenter() 不存在，手动构造方块中心 Vec3d
                Vec3d netherCenter = new Vec3d(netherBase.getX() + 0.5, netherBase.getY() + 0.5, netherBase.getZ() + 0.5);
                double dist = client.player.getPos().distanceTo(netherCenter);
        ...
        // 1.20.1: client.textRenderer 获取字体渲染器
        ...
        // 绘制半透明黑色背景（帮助调试和阅读）
        drawContext.fill(bgX, bgY, bgX + bgW, bgY + bgH, 0xCC000000);

        for (int i = 0; i < lines.size(); i++) {
            int y = startY + i * (LINE_HEIGHT + 2);
            // 1.20.1: drawTextWithShadow(textRenderer, text, x, y, color)
            drawContext.drawTextWithShadow(font, Text.literal(lines.get(i)), startX, y, 0xFFFFFFFF);
        }
    }
